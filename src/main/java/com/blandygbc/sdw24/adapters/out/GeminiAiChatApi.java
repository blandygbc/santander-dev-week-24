...
public interface GeminiAiChatApi extends GenerativeAiService {

    static final Logger logger = LoggerFactory.getLogger(GeminiAiChatApi.class);
    ...
    default String generateContent(String objective, String context) {
        ...
        try {
            ...
            logger.error(
                    "Deu ruim! Erro de comunicação com a API do Google Gemini.",
                    httpErrors);
            return "Deu ruim! Erro de comunicação com a API do Google Gemini.";
        } catch (Exception unexpectedError) {
            logger.error(
                    "Deu mais ruim ainda! O retorno da API do Google Gemini não contem os dados esperados.",
                    unexpectedError);
